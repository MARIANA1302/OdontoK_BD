...
class UserController
{
    ...
    function insertUser()
    {
        ...
        if (
            ...
            $cod_state == 'Seleccione un estado' || $cod_role == "Seleccione un rol"
        ) {
        ...
        $array_states = $UserModel->paginateState();
        $UserView = new UserView();
        $UserView->paginateUsers($array_user, $array_states);
    }
    ...
    function searchUsersDocument()
    {
        ...
        if ($document_number == "") {
            ...
            if(!(ctype_digit($document_number)) || strlen($document_number) != 10){
                ...
                $UserView->paginateUsers($array_users,$array_states);
            }
    ...
    function searchUsersState(){
        ...
        if($cod_state == "todo"){
            ...
            $array_users = $UserModel->searchUserState($cod_state);
        }
        $UserView->paginateUsers($array_users,$array_states);
    }
    ...
    function showFormUser()
    {
        ...
        $arrayState = $UserModel->paginateState();
        //Crear y mostrar el formulario
        $UserView = new UserView();
        $UserView->showFormUser($arrayTypeDocument, $arrayRol, $arrayState);
    }
    ...
    function showUser(){
        ...
        $array_state = $UserModel->paginateState();
        $array_type_document = $UserModel->paginateDocumentType();
        //Cargar y mostrar vistas
        $UserView = new UserView();
        $UserView->showUser($user,$array_type_document,$array_role,$array_state);
    ...
    function updateUser(){
        ...
        if (
            ...
            $array_message = ['message' => 'Error, las contraseñas no coinciden'];
            exit(json_encode($array_message));
            //Saber si el documento que se esta ingresando ya esta asignado
        } else if ($array_document_number) {
            $array_message = ['message' => 'Ya existe un usuario con ese documento'];
